...
const Blog = (match) => {
	...
	/*const getPosts = async () => {
		await axios.get('/api/post/all').then((res) => {
	...
	const dateFunction = (date) => {
		...
		const diffDays = parseInt(
			(new Date(now) - new Date(formattedDate)) / (1000 * 60 * 60 * 24),
			10
		);
		...
		if (diffDays >= 365)
			return `${Math.floor(diffDays / 365)} ${
				Math.floor(diffDays / 365) === 1 ? 'year' : 'years'
			} ago`;
	};
